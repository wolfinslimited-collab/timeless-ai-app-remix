...
class ChatService {
  ...
  Future<List<Conversation>> getConversations() async {
    ...
    if (user == null) {
      logger.warning(
          'Cannot fetch conversations: not authenticated', 'SUPABASE');
      return [];
    }
    ...
    try {
      ...
      logger.success(
          'Fetched ${(response as List).length} conversations', 'SUPABASE');
  ...
  Future<List<Map<String, dynamic>>> getFolders() async {
    ...
    try {
      ...
      logger.success(
          'Fetched ${(response as List).length} folders', 'SUPABASE');
  ...
  Future<bool> deleteFolder(String id) async {
    ...
    try {
      ...
      await _supabase
          .from('conversations')
          .update({'folder_id': null}).eq('folder_id', id);
  ...
  Future<bool> moveToFolder(String conversationId, String? folderId) async {
    logger.info(
        'Moving conversation $conversationId to folder: $folderId', 'SUPABASE');
    try {
      await _supabase
          .from('conversations')
          .update({'folder_id': folderId}).eq('id', conversationId);
  ...
  Future<void> togglePin(String id, bool pinned) async {
    logger.info(
        '${pinned ? 'Pinning' : 'Unpinning'} conversation: $id', 'SUPABASE');
    await _supabase
        .from('conversations')
        .update({'pinned': pinned}).eq('id', id);
