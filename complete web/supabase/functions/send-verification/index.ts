...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!email || !fullName || !password) {
      throw new Error("Missing required fields: email, fullName, and password are required");
    }
    ...
    // Use listUsers with filter to check for specific email
    ...
    // Since listUsers doesn't support email filter directly, we need to check differently
